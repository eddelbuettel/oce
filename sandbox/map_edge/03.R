library(oce)
data(coastlineWorld)
if (!interactive()) pdf("03.pdf", height=4, pointsize=9)
...
plot(res$lon, res$lat, type="o", pch=20, cex=1/2, col=2, xlab="lon", ylab="lat", asp=1)
...
CLp <- sp::Polygon(cbind(lonlat$lon[brokenBorder], lonlat$lat[brokenBorder]))
