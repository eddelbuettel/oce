  ...
  for (int i = 0; i < nbuf - 3 - chunk_length; i++) { // FIXME is 3 right, or needed?
    if ((is_adp || is_pcadp) && buf[i] == adpByte1 && buf[i+1] == adpByte2 && buf[i+2] == adpByte3) {
      ...
      if (check_sum == desired_check_sum) {
        matches++;
#ifdef DEBUG
        if (i < 10 * chunk_length)
          Rprintf("OK  at buf[%d]: check_sum=%d (should be %d); check_sum_start=%d (will stop reporting at buf[%d])\n",
              i, check_sum, desired_check_sum, check_sum_start, 10*chunk_length);
#endif
...
#ifdef DEBUG
        if (i < 10 * chunk_length)
          Rprintf("BAD at buf[%d]: check_sum=%d (should be %d); check_sum_start=%d (will stop reporting at buf[%d])\n",
              i, check_sum, desired_check_sum, check_sum_start, 10*chunk_length);
#endif
        if (bad++ > maxbad)
