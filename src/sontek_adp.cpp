...
char *test_message = "# R test on a particular argonaut file, based on byte pairs\n"
"# in Sontek docs ... *but* those docs are for an adv, not\n"
"# an adp so I think they are irrelevant -- DEK.\n"
"file <- '~/Dropbox/oce-issue-1637/file.arg'\n"
"nbuf <- file.info(file)$size # 3013196\n"
"buf <- readBin(file, 'raw', nbuf)\n"
"# How many 0xB1 0x16 (short) records, according to Sontek doc on such?\n"
"short1 <- which(buf == as.raw(0xB1))\n"
"short2 <- which(buf[short1 + 1] == as.raw(0x16))\n"
